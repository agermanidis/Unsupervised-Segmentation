...
@runway.command('segment', inputs=command_inputs, outputs={'segmented_image': runway.image})
def run(_, inputs):
    model = MyNet(inp_dim=3, mod_dim1=64, mod_dim2=32).to(device)
    ...
    color_avg = np.random.randint(255, size=(inputs['max_labels'], 3))
    show = image

    model.train()
    for batch_idx in range(inputs['iterations']):
        '''forward'''
        optimizer.zero_grad()
        output = model(tensor)[0]
        output = output.permute(1, 2, 0).view(-1, 32)
        ...
        if un_label.shape[0] < inputs['max_labels']:  # update show
        ...
        if len(un_label) < inputs['min_labels']:
            break
